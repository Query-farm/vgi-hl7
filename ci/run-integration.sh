...
#   VGI_HL7_WORKER    worker LOCATION the .test files attach (a stdio command
#                      such as `java -jar /abs/path/vgi-hl7-<ver>-all.jar`, or
#                      an http:// URL)
# Optional:
...
: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_HL7_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
# The .test files may reference committed fixtures under test/sql/data via
# relative paths; stage them alongside the preprocessed tests so the runner
# (which cd's into $STAGE) resolves them.
...
echo "Running suite (worker: $VGI_HL7_WORKER) ..."
