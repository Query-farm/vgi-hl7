...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-hl7
...
#   subprocess  (default) — VGI_HL7_WORKER is a stdio command; DuckDB spawns
...
#   VGI_HL7_WORKER    for subprocess: the stdio command (e.g.
#                      `java -jar /abs/path/vgi-hl7-<ver>-all.jar`).
#                      For http/unix it is computed here, so set
#                      VGI_HL7_WORKER_JAR (or VGI_HL7_WORKER as the bare jar
#                      command) instead — see below.
# Optional:
#   TRANSPORT          subprocess | http | unix          (default: subprocess)
#   VGI_HL7_WORKER_JAR  abs path to the fat JAR (required for http/unix; if
#                        unset, derived from VGI_HL7_WORKER by stripping a
#                        leading `java -jar `).
...
# ---------------------------------------------------------------------------
# Bring up the worker transport and export VGI_HL7_WORKER for the .test files.
# ---------------------------------------------------------------------------
WORKER_PID=""
cleanup() {
  rc=$?
...
# Resolve the fat JAR command for http/unix. Prefer VGI_HL7_WORKER_JAR; else
# strip a leading `java -jar ` from VGI_HL7_WORKER.
resolve_jar() {
  if [ -n "${VGI_HL7_WORKER_JAR:-}" ]; then
    printf '%s' "$VGI_HL7_WORKER_JAR"
    return
  fi
  if [ -n "${VGI_HL7_WORKER:-}" ]; then
    printf '%s' "${VGI_HL7_WORKER#java -jar }"
    return
  fi
  echo "::error::http/unix transport needs VGI_HL7_WORKER_JAR (or VGI_HL7_WORKER as the jar command)" >&2
  exit 1
}

case "$TRANSPORT" in
  subprocess)
    : "${VGI_HL7_WORKER:?worker LOCATION (stdio command) for subprocess transport}"
    echo "subprocess worker: $VGI_HL7_WORKER"
    ;;
  ...
  http)
    ...
    export VGI_HL7_WORKER="http://127.0.0.1:$PORT"
    ;;

  unix)
    JAR="$(resolve_jar)"
    SOCK="${UNIX_SOCK:-$STAGE/vgi-hl7.sock}"
    ...
    export VGI_HL7_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_HL7_WORKER) ..."
OUT="$STAGE/suite.out"
set +e
