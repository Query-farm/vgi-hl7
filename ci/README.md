# CI: the vgi-hl7 worker integration suite
...
(`test/sql/*.test`) against the vgi-hl7 VGI worker through the **real DuckDB
...
1. **Build the worker** — `./gradlew shadowJar` produces a self-contained fat
   JAR at `build/libs/vgi-hl7-<ver>-all.jar`. Its manifest sets `Add-Opens`, so
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   ...
   chosen transport, points `VGI_HL7_WORKER` at it, **warms the extension cache
...
`VGI_HL7_WORKER` resolves to. `run-integration.sh` is parameterized by a
...
| `TRANSPORT`  | `VGI_HL7_WORKER`       | how the worker is started | readiness gate |
|--------------|-------------------------|---------------------------|----------------|
...
The suite itself is transport-agnostic — the table / table-in-out functions
emit their result and `finish()` within a single producer tick (or, for the
table-in-out writers, re-emit the accumulated result after each input batch), so
the worker never has to resume a producer mid-stream across an HTTP continuation
boundary, and the scalar functions are pure request/response. No test needs a
per-transport gate; all three legs run the full suite.
...
VGI_HL7_WORKER_JAR="$JAR" ci/run-integration.sh
